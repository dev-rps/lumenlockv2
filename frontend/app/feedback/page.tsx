      ...
      >
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          ...
          <p className="text-white/80 text-sm max-w-xl mx-auto">
            Help us improve LumenLock. Your feedback directly shapes our smart contract escrow roadmap.
          </p>
          ...
          <div className="flex items-center justify-center gap-8 mt-4 pt-2">
            {[
              ...
              { label: "Average Rating", value: +avgRating.toFixed(1), suffix: "★" },
      ...
      <section className="max-w-6xl mx-auto px-6 py-6 md:py-8 grid md:grid-cols-2 gap-8">
        ...
        <div>
          <div className="mb-4">
            ...
            <p className="text-xs" style={{ color: "var(--fg-muted)" }}>
              All responses are stored securely in our Neon PostgreSQL cloud database.
            </p>
          </div>
          <div
            className="p-5 rounded-2xl"
            style={{ background: "var(--surface-0)", border: "1px solid var(--border-subtle)", boxShadow: "var(--shadow-md)" }}
          >
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-xs font-bold" style={{ color: "var(--success-text)" }}>Live Feedback Dataset Export</p>
              <p className="text-[11px] mt-0.5" style={{ color: "var(--success-text)", opacity: 0.85 }}>
                Download latest feedback dataset (CSV format, newest on top)
              </p>
            </div>
            <a
              href="/api/feedback/export"
              download="user_feedback_dataset.csv"
              className="px-3.5 py-2 rounded-xl text-xs font-bold text-white transition-all duration-150 active:scale-[0.97] shadow-sm hover:opacity-90"
        ...
        <div>
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold mb-1" style={{ color: "var(--fg-default)" }}>Recent Feedback & Testimonials</h2>
              <p className="text-xs" style={{ color: "var(--fg-muted)" }}>
                Latest user submissions from Neon Postgres cloud database.
              </p>
            </div>
            <span className="text-xs font-semibold px-2.5 py-1 rounded-lg" style={{ background: "var(--primary-50)", color: "var(--primary-600)" }}>
              Newest First
            </span>
          </div>
          ...
          <div className="space-y-3 max-h-[560px] overflow-y-auto pr-2">
            {liveFeedback.length > 0
              ? liveFeedback.map((fb) => (
                  <div
                    key={fb.id}
                    className="p-4 rounded-2xl transition-all duration-150 hover:-translate-y-0.5"
                    style={{ background: "var(--surface-0)", border: "1px solid var(--border-subtle)", boxShadow: "var(--shadow-xs)" }}
                  >
                    <div className="flex items-start gap-3">
                      <AvatarInitials name={fb.name} />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <p className="font-semibold text-xs truncate" style={{ color: "var(--fg-default)" }}>{fb.name}</p>
                          <div className="flex shrink-0">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <svg
                                ...
                                height="12"
                                viewBox="0 0 24 24"
                                fill={i < fb.overallRating ? "#FBBF24" : "none"}
                                stroke={i < fb.overallRating ? "#FBBF24" : "var(--border-default)"}
                                strokeWidth="1.5"
                              >
                        ...
                        <div className="flex items-center gap-2 text-[10px] mt-0.5 mb-1" style={{ color: "var(--fg-subtle)" }}>
                          <span>{fb.network || "Stellar Testnet"}</span>
                          {fb.recommend && (
                            <>
                              <span>•</span>
                              <span>Recommend: <strong>{fb.recommend}</strong></span>
                            </>
                          )}
                          {fb.submittedAt && (
                            <>
                        ...
                        {fb.bugsReported && fb.bugsReported !== "N/A" && (
                          <p className="text-[11px] mt-1 p-2 rounded-lg" style={{ background: "var(--surface-1)", color: "var(--fg-default)" }}>
                            <strong>Bug/Issue:</strong> {fb.bugsReported}
                          </p>
                        )}
                        {fb.improvements && (
                          <p className="text-[11px] mt-1 p-2 rounded-lg" style={{ background: "var(--surface-1)", color: "var(--fg-default)" }}>
                            <strong>Suggested Improvement:</strong> {fb.improvements}
                          </p>
                        )}
                      </div>
                    </div>
              ...
              : INDIAN_TESTERS.map((t) => (
                  <div
                    key={t.name}
                    className="p-3.5 rounded-xl transition-all duration-150 hover:-translate-y-0.5"
                    style={{ background: "var(--surface-0)", border: "1px solid var(--border-subtle)", boxShadow: "var(--shadow-xs)" }}
                  >
                    <div className="flex items-start gap-3">
                          ...
                          <div className="flex shrink-0">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <svg
                                ...
                                stroke={i < t.rating ? "#FBBF24" : "var(--border-default)"}
                                strokeWidth="1.5"
                              >
                                <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                              </svg>
